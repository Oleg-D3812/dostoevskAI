(function () {
  ...
  fetchJson(DATA_ROOT + "manifest.json").then(function (root) {
    ...
    return fetchJson(DATA_ROOT + novel.manifest).then(function (manifest) {
      ...
      document.title = manifest.title + " — VAD облака глав";
  ...
  function bindControls() {
    ...
    document.getElementById("style-reset").addEventListener("click", function () { document.getElementById("grid-width").value = 1; document.getElementById("grid-opacity").value = .75; document.getElementById("grid-color").value = "#697078"; document.getElementById("grid-dash").value = "solid"; document.getElementById("plane-opacity").value = .35; document.querySelectorAll("output[data-for]").forEach(function (o) { o.value = document.getElementById(o.dataset.for).value; }); draw(); });
  }
  function refresh() {
  ...
  function draw() {
    var traces = frame(); state.sphereIndices = [];
